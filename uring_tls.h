...
void prep_write_fixed(int fd, struct io_uring* ring, __u16 idx, int type, const void *buffer,
                      int size, int buff_idx) {
    struct io_uring_sqe *sqe = io_uring_get_sqe(ring);
    io_uring_prep_write_fixed(sqe, fd, buffer, size, 0, buff_idx);
...
void on_data_recv(struct io_uring *uring, struct io_uring_cqe *cqe, struct read_buff_pool *read_buff_pool) {
    struct uring_user_data  data;
    ...
    void *buffer = buffer_pool_get(read_buff_pool, buf_idx);
    BIO *read_bio = SSL_get_rbio(conn->ssl);
    BIO_write(read_bio, buffer, read_bytes);

    const unsigned int max_buffer_size = read_buff_pool->buff_size;

    on_recv_decrypted_data(conn,data.conn_idx, uring);

    buffer_pool_release(read_buff_pool, buf_idx);
...
int do_ssl_handshake(struct io_uring_cqe *cqe, struct io_uring *ring,
    ...
    else if (ssl_err == 0 && SSL_is_init_finished(ssl)) {
        return 0;
    }
    else {
    ...
    switch (tag.req_type) {
        ...
        case DATA_READ: {
            if (cqe->res <= 0) {
                ...
                on_data_recv(ring, cqe, read_buff_pool);
            }
            break;
        }
        case DATA_WRITE: {
